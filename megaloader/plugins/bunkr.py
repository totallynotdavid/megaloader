...
class Bunkr(BasePlugin):
    ...
    def _solve_encrypted_url(
        ...
        try:
            ...
            print(f"[ERROR] Bunkr: Error decrypting URL: {e}")
            return None
    ...
    def export(self) -> Generator[str, None, None]:
        try:
            ...
            if "/a/" in resolved_url:
                ...
                if not links:
                    print(
                        "[WARNING] Bunkr: Could not find any file links on the album page."
                    )
                    return
                print(f"[INFO] Bunkr: Found {len(links)} items in album.")
                unique_urls = set()
            ...
            # Case 3: We landed directly on a final download page (/file/...).
                ...
                print(
                    f"[WARNING] Bunkr: Unrecognized URL format after redirect: {resolved_url}"
                )
        except requests.RequestException as e:
            print(f"[ERROR] Bunkr: Error fetching initial URL {self.url}: {e}")
    ...
    def _resolve_viewer_page(self, viewer_url: str) -> Generator[str, None, None]:
        ...
        try:
            ...
            if download_button and download_button.get("href"):
                ...
                print(
                    f"[WARNING] Bunkr: Failed to find download button on viewer page {viewer_url}"
                )
        except requests.RequestException as e:
            print(f"[ERROR] Bunkr: Failed to resolve viewer page {viewer_url}: {e}")
                ...
                with open(save_path, "wb") as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        if chunk:  # Filter out keep-alive chunks
                            f.write(chunk)
            print(f"[SUCCESS] Bunkr: Downloaded '{original_filename}'")
            return save_path
        except Exception as e:
            print(f"[ERROR] Bunkr: Failed to download from {file_page_url}: {e}")
            return None
